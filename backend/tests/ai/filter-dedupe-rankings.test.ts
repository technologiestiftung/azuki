...
describe("filterAndDedupeRankings", () => {
	test("drops rankings whose id is not in the valid set", () => {
		const rankings = [
			...
			{ id: 999, kurzdefinition: "", begruendung: "rogue" },
			{ id: 2, kurzdefinition: "", begruendung: "b" },
		];
		...
		const rankings = [
			{
				id: 9910,
				kurzdefinition: "",
				begruendung: "Friseur passt zum Salon-Praktikum",
			},
			{ id: 14624, kurzdefinition: "", begruendung: "Kosmetiker dual" },
			{
				id: 9910,
				kurzdefinition: "",
				begruendung: "(LLM emitted duplicate — drop this)",
			},
			{ id: 33212, kurzdefinition: "", begruendung: "MFA als Plan B" },
		];
	...
	test("preserves order of the first occurrence of each valid id", () => {
		const rankings = [
			{ id: 3, kurzdefinition: "", begruendung: "c" },
			{ id: 1, kurzdefinition: "", begruendung: "a" },
			{ id: 2, kurzdefinition: "", begruendung: "b" },
		];
		const validIds = new Set([1, 2, 3]);
	...
	test("returns empty array when no rankings are valid", () => {
		const rankings = [
			{ id: 100, kurzdefinition: "", begruendung: "x" },
			{ id: 200, kurzdefinition: "", begruendung: "y" },
		];
		const validIds = new Set([1, 2, 3]);
		expect(filterAndDedupeRankings(rankings, validIds)).toEqual([]);
	});
	...
	test("cap counts valid, unique rankings — duplicates and rogue ids don't consume a slot", () => {
		const rankings = [
			{ id: 1, kurzdefinition: "", begruendung: "a" },
			{ id: 999, kurzdefinition: "", begruendung: "rogue" },
			{ id: 1, kurzdefinition: "", begruendung: "dup" },
			{ id: 2, kurzdefinition: "", begruendung: "b" },
			{ id: 3, kurzdefinition: "", begruendung: "c" },
		];
		const validIds = new Set([1, 2, 3]);
