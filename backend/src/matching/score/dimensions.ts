...
export function scoreNoGos(occupation: Occupation, profile: UserProfile): number {
	let penalty = 0;
...
export function scoreStrengths(occupation: Occupation, profile: UserProfile): number {
	let score = 0;
	...
	for (const [strengthId, value] of Object.entries(profile.strengths)) {
		...
		if (strengthId === "craftsmanship") {
			...
			if (occupation.conditions.manualLabor || occupation.conditions.machinery) {
				score += 2;
			}
			continue;
		}
...
export function scoreWorkValues(
	...
	for (const valueId of profile.workValues ?? []) {
		if (valueId === "good_salary") {
			if (!occupation.salaryKnown || occupation.salaryMonthlyMedian === null) continue;
			if (!salaryBands) continue;

			if (occupation.salaryMonthlyMedian >= salaryBands.upperBandMin) score += 3;
